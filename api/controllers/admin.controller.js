...
exports.addProvider = async (req, res) => {
    ...
    const {name, email, phone, address, fax, introduce, service, travler, revenue, prize, images} = req.body;
   ...
   let uploadImg;
    ...
    let providerFind = null, userFind = null;
    try {
        userFind = await user.find({ 'email': email });
        providerFind = await provider.find({ 'email': email });
    }
    catch (err) {
...
exports.updateUser = async (req, res) => {
    ...
    res.status(200).json({ msg: 'success', data: userFind});
}
...
exports.login = async (req, res) => {
    ...
    let userFind = null; //role =2 for admin
    try{
        userFind = await user.findOne({'email': email, 'role': 2});
    }
    catch(err){
        res.json({msg: err});
        return;
