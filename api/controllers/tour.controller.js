...
exports.getAllTour = async (req, res) => {
    ...
    try {
        if (range !== null) {
            tourCount = await tour
                .count({ $or: [{ name: new RegExp(searchText, "i") }, { provider_id: { $in: searchProvider } }, { category_tour_id: { $in: searchCategory } }], price: { $gte: objRange.low, $lte: objRange.high } });
        }
        else {
            tourCount = await tour.count({ $or: [{ name: new RegExp(searchText, "i") }, { provider_id: { $in: searchProvider } }, { category_tour_id: { $in: searchCategory } }] });
        }
    }
    catch (err) {
        res.status(500).json({ msg: err });
    ...
    if (range !== null) {
        tour
            .find({ $or: [{ name_tour: new RegExp(searchText, "i") }, { provider_id: { $in: searchProvider } }, { category_tour_id: { $in: searchCategory } }], price: { $gte: objRange.low, $lte: objRange.high } })
        ...
        tour
            .find({ $or: [{ name_tour: new RegExp(searchText, "i") }, { provider_id: { $in: searchProvider } }, { category_tour_id: { $in: searchCategory } }] })
            .skip(9 * (parseInt(page) - 1))
            .limit(9)
            .sort(sortQuery)
            .exec((err, docs) => {
